/**
 * Password hashing and policy — runtime-agnostic core.
 *
 * Lives apart from password.ts so callers that run OUTSIDE the Next.js
 * server runtime (notably the Prisma seed via `tsx`, and the vitest unit
 * suite) can import the same Argon2id parameters and policy without
 * dragging the `server-only` directive — which throws at module load
 * outside the React Server Components context.
 *
 * Server runtime code should keep importing from `password.ts`, which is
 * a thin server-only re-export of this module. That preserves the
 * "this code must not reach the client bundle" contract for the Next
 * server, while letting one-shot Node entry points (seed, tests) work.
 *
 * Argon2id parameters and the breached-password blocklist are identical
 * in both files. There is one source of truth — this file.
 */
