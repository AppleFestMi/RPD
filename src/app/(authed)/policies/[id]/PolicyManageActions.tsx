...
import { archivePolicy, publishPolicy, unpublishPolicy } from "../actions";
import type { PolicyStatus } from "@/lib/policies/types";

export function PolicyManageActions({
  policyId,
  status,
  canPublish,
  canManage,
    ...
    <div className="space-y-2 text-[13px]">
      {status === "draft" && canPublish ? (
        <button
          ...
          onClick={() => wrap(() => publishPolicy({ policyId }))}
          title={!hasFile ? "Attach a document before publishing." : ""}
      ...
      {status === "published" && canPublish ? (
        <button
          type="button"
          disabled={pending}
          onClick={() => wrap(() => unpublishPolicy({ policyId }))}
        ...
        !confirmArchive ? (
          ...
          <div className="space-y-2 rounded-md border border-danger/40 bg-danger-soft/30 p-2">
            <p className="text-[12.5px] text-text2">
              Archiving hides this policy from non-managers. The audit trail and the document
              file are preserved.
            </p>
            <div className="flex gap-2">
              <button
                ...
                onClick={() => wrap(() => archivePolicy({ policyId }))}
              >
                Confirm archive
              </button>
              <button
